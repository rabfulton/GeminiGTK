...
class ChatWindow(Gtk.ApplicationWindow):
    ...
    def _insert_inline_markup(self, iter_start: Gtk.TextIter, text: str) -> None:
        ...
        for match in pattern.finditer(text):
            ...
            if start > position:
                self._insert_basic_markup(iter_start, text[position:start])

            formula = match.group(2).strip()
            is_block = len(match.group(1)) == 2
            inserted = self._insert_latex(iter_start, formula, is_block)
            if not inserted:
                self._insert_basic_markup(iter_start, match.group(0))
            position = end

        if position < len(text):
            self._insert_basic_markup(iter_start, text[position:])

    def _insert_basic_markup(self, iter_start: Gtk.TextIter, text: str) -> None:
    ...
    def _insert_latex(self, iter_start: Gtk.TextIter, formula: str, block: bool) -> bool:
        ...
        if block:
            self.textbuffer.insert(self.textbuffer.get_end_iter(), "\n")
        self.textbuffer.insert_pixbuf(iter_start, pixbuf)
        if block:
            self.textbuffer.insert(self.textbuffer.get_end_iter(), "\n")
        return True
